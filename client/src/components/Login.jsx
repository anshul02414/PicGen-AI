  ...
  return (
    <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      ...
      <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        {
            state == 'Login' &&
            <p className='text-sm text-blue-600 my-4 cursor-pointer mb-5'>
                Forgot Password
            </p>
        }

        <button className='bg-blue-600 w-full text-white py-2 rounded-full cursor-pointer mt-5'>
...

        {
            ...
            Dont't have an account? <span className='text-blue-600 cursor-pointer' onClick={()=>{setState('Sign Up')}}>Sign Up</span></p>
            :
            <p className='mt-5 text-center'>
            Already Have an Account <span className='text-blue-600 cursor-pointer' onClick={()=>{setState('Login')}}>Log in</span></p>
        }
